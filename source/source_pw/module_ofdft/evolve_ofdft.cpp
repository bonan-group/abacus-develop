...
#pragma omp parallel for collapse(2)
    ...
    this->cal_tf_potential(chr.rho, pw_rho, pelec->pot->get_eff_v_for_write()); // TF potential
    if (PARAM.inp.of_cd)
    {
        this->cal_CD_potential(psi_,
                               pw_rho,
                               pelec->pot->get_eff_v_for_write(),
                               PARAM.inp.of_mCD_alpha); // CD potential
    }
