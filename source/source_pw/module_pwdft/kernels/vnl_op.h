...
/// @brief Calculate vkb for a range of atoms using pre-computed gk, ylm, and sk_all (cached)
///
/// This kernel is similar to cal_vnl_atoms_op but accepts pre-computed gk, ylm,
/// and sk_all arrays that are cached at k-point level. No per-chunk sk computation needed.
///
/// @tparam FPTYPE Floating point type (float or double)
/// @tparam Device Device type (CPU or GPU)
template <typename FPTYPE, typename Device>
struct cal_vnl_atoms_cached_op
{
    ...
    /// @param tpiba - ucell.tpiba
    /// @param NEG_IMAG_UNIT - ModuleBase::NEG_IMAG_UNIT
    /// @param gk - PRE-COMPUTED G+k vectors (size: npw * 3) - cached at k-point level
    /// @param ylm - PRE-COMPUTED spherical harmonics (size: x1 * npw) - cached at k-point level
    /// @param indv - pseudopot_cell_vnl::indv
    /// @param nhtol - pseudopot_cell_vnl::nhtol
    /// @param nhtolm - pseudopot_cell_vnl::nhtolm
    /// @param tab - pseudopot_cell_vnl::tab
    ...
    void operator()(const Device* ctx,
                    ...
                    const FPTYPE* tab,
                    FPTYPE* vkb1,
                    const std::complex<FPTYPE>* sk_all,  // All-atom sk (nat * npw), cached at k-point level
                    const int* iat2it,
                    std::complex<FPTYPE>* vkb_out);
};
