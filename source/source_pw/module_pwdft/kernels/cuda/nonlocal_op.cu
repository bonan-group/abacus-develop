    ...
    std::complex<FPTYPE>* qgm)
{
    ...
    uspp_qgm_build<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        ntype,
        nh_tot,
        npw,
        lmaxq,
    ...
    const std::complex<FPTYPE>* becp)
{
    ...
    uspp_overlap<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        atom_count,
        nbands,
        nh,
        nhm,
    ...
    FPTYPE* deeq)
{
    ...
    uspp_deeq<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        nspin,
        atom_count,
        nh,
        nhm,
    ...
    FPTYPE* force)
{
    ...
    uspp_force<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        nspin,
        atom_count,
        nij,
        npw,
...
{
    const int count = atom_count * 3;
    uspp_stress<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        nspin,
        atom_count,
        nij,
        npw,
