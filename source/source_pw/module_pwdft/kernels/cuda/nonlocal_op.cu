...
void uspp_qgm_build_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
    ...
    std::complex<FPTYPE>* qgm)
{
    ...
    nonlocal_gpu_detail::uspp_qgm_build<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        ntype,
        nh_tot,
        npw,
        lmaxq,
...
template <typename FPTYPE>
void uspp_overlap_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
    const base_device::DEVICE_GPU* /*dev*/,
    int atom_count,
    int nbands,
    int nh,
    ...
    const std::complex<FPTYPE>* becp)
{
    ...
    nonlocal_gpu_detail::uspp_overlap<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        atom_count,
        nbands,
        nh,
        nhm,
...
template <typename FPTYPE>
void uspp_deeq_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
    const base_device::DEVICE_GPU* /*dev*/,
    int nspin,
    int atom_count,
    int nh,
    ...
    FPTYPE* deeq)
{
    ...
    nonlocal_gpu_detail::uspp_deeq<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        nspin,
        atom_count,
        nh,
        nhm,
...
template <typename FPTYPE>
void uspp_force_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
    const base_device::DEVICE_GPU* /*dev*/,
    int nspin,
    int atom_count,
    int nij,
    ...
    FPTYPE* force)
{
    ...
    nonlocal_gpu_detail::uspp_force<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        nspin,
        atom_count,
        nij,
        npw,
...
template <typename FPTYPE>
void uspp_stress_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
    const base_device::DEVICE_GPU* /*dev*/,
    int nspin,
    int atom_count,
    int nij,
...
{
    const int count = atom_count * 3;
    nonlocal_gpu_detail::uspp_stress<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        nspin,
        atom_count,
        nij,
        npw,
                                                                         ...
                                                                         const std::complex<FPTYPE>* becp)
{
  ...
  // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  nonlocal_gpu_detail::nonlocal_pw<FPTYPE><<<l1 * l2, THREADS_PER_BLOCK>>>(
    l1, l2, l3, // loop size
                                                                         ...
                                                                         const std::complex<FPTYPE>* becp)
{
  ...
  // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  nonlocal_gpu_detail::nonlocal_pw<FPTYPE><<<l1 * l2 / 2, THREADS_PER_BLOCK>>>(
    l1, l2, l3, // loop size
