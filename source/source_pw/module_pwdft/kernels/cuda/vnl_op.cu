...
template<typename FPTYPE>
__global__ void cal_vnl_atoms_cached(const int ntype,
                                     const int npw,
                                     const int npwx,
                                     const int nhm,
                                     const int tab_2,
...
template <typename FPTYPE>
void cal_vnl_atoms_cached_op<FPTYPE, base_device::DEVICE_GPU>::operator()(const base_device::DEVICE_GPU* ctx,
                                                                          const int& ntype,
                                                                          const int& npw,
                                                                          const int& npwx,
                                                                          const int& nhm,
...
template<typename FPTYPE>
__global__ void cal_hpsi_from_vkb1_cache(const int npw,
                                         const int npwx,
                                         const int nbands,
                                         const int nkb,
                                         const int nhm,
                                         ...
                                         const FPTYPE* vkb1_cache,
                                         const thrust::complex<FPTYPE>* sk_all,
                                         const thrust::complex<FPTYPE>* ps,
...
template <typename FPTYPE>
void cal_hpsi_from_vkb1_cache_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
    const base_device::DEVICE_GPU* ctx,
    const int& npw,
    const int& npwx,
    const int& nbands,
...
{
    const dim3 grid((npw + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, nbands);
    cal_hpsi_from_vkb1_cache<FPTYPE><<<grid, THREADS_PER_BLOCK>>>(
        npw,
        npwx,
        nbands,
        nkb,
        ...
        vkb1_cache,
        reinterpret_cast<const thrust::complex<FPTYPE>*>(sk_all),
        reinterpret_cast<const thrust::complex<FPTYPE>*>(ps),
...
template struct cal_hpsi_from_vkb1_cache_op<float, base_device::DEVICE_GPU>;
template struct cal_hpsi_from_vkb1_cache_op<double, base_device::DEVICE_GPU>;
