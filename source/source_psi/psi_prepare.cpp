                                        ...
                                        std::ofstream& ofs_running)
{
    ...
    const psi_init_atomic<T>* atomic_initializer = dynamic_cast<const psi_init_atomic<T>*>(this->psi_initer.get());
    AtomicGpuInitializer<T> atomic_gpu_initializer(
        atomic_initializer,
        this->pw_wfc,
        this->ucell,
        this->sf,
        gpu_init_policy,
        random_seed);
    const bool random_gpu_init = this->psi_initer->method() == "random";
    const bool gpu_resident_init = gpu_init_policy && (random_gpu_init || atomic_gpu_initializer.available());
    ...
    for (int ik = 0; ik < this->pw_wfc.nks; ik++)
    {
        ...
        if (fill)
        {
            ...
            if (gpu_resident_init)
            {
                ...
                {
                    atomic_gpu_initializer.initialize_k(psi_device->get_pointer(), nbands_start, ik, ik_tot);
                }
            }
            else
            {
