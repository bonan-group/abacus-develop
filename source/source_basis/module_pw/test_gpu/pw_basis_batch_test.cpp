...
/**
 * @brief Test suite for PW_Basis batch FFT transforms
 *
 * Tests batch versions of recip_to_real and real_to_recip against
 * sequential implementations to verify correctness.
 *
 * This tests the density transform batch FFT (PW_Basis), which is
 * simpler than wavefunction transforms (PW_Basis_K) since there are
 * no k-point variations.
 */
template <typename T>
class PW_BASIS_BATCH_TEST : public ::testing::Test
{
  public:
    using Real = T;
    ...
    void SetUp() override
    {
        // CRITICAL: Set device and precision BEFORE setuptransform()
        ...
        // Initialize PW_Basis (no k-points needed)
        ...
        base_device::memory::resize_memory_op<complex<T>, Device>()(d_out_batch_seq, batch_size * npw);
        base_device::memory::resize_memory_op<complex<T>, Device>()(d_out_batch, batch_size * npw);
        base_device::memory::resize_memory_op<complex<T>, Device>()(d_tmp_seq, nrxx);

        // Allocate host memory for verification
        h_out_seq = new complex<T>[batch_size * npw];
        h_out_batch = new complex<T>[batch_size * npw];
    }
...
TYPED_TEST_SUITE(PW_BASIS_BATCH_TEST, TestTypes);

/**
 * @brief Test batch FFT availability after proper initialization
 *
 * This is the KEY TEST - verifies that batch FFT is properly initialized
 * for PW_Basis after calling setuptransform() with correct device/precision.
 */
TYPED_TEST(PW_BASIS_BATCH_TEST, BatchFFTAvailable)
{
    using T = typename TestFixture::Real;

    // Verify batch FFT is available
    bool is_available = this->pwtest.fft_bundle.template is_batch_fft_available<T>();

    EXPECT_TRUE(is_available)
        << "Batch FFT should be available after proper initialization. "
        << "Device=" << this->pwtest.get_device()
        << ", Precision=" << (typeid(T) == typeid(double) ? "double" : "float");

    // If this test fails, it indicates the root cause of the performance issue:
    // batch FFT is not being properly initialized for PW_Basis density transforms
}

TYPED_TEST(PW_BASIS_BATCH_TEST, RecipToRealBatch)
{
    using T = typename TestFixture::Real;
    ...
    base_device::memory::resize_memory_op<complex<T>, Device>()(d_in_recip_batch, batch_count * npw);
    this->FillRandomData(d_in_recip_batch, batch_count * npw);
    ...
    // Sequential transforms (template signature: TK, TR, Device - no ik parameter)
    for (int ib = 0; ib < batch_count; ib++)
    {
        this->pwtest.template recip_to_real<std::complex<T>, std::complex<T>, Device>(
            d_in_recip_batch + ib * npw,
            d_out_real_seq + ib * nrxx,
            false,
            T(1.0));
    }

    // Batch transform (no ik_batch parameter - simpler!)
...
TYPED_TEST(PW_BASIS_BATCH_TEST, RealToRecipBatch)
{
    using T = typename TestFixture::Real;
    using Device = typename TestFixture::Device;
    ...
    base_device::memory::resize_memory_op<complex<T>, Device>()(d_out_recip_seq, batch_count * npw);
    base_device::memory::resize_memory_op<complex<T>, Device>()(d_out_recip_batch, batch_count * npw);

    // Sequential transforms (template signature: TK, TR, Device - no ik parameter)
    for (int ib = 0; ib < batch_count; ib++)
    {
        this->pwtest.template real_to_recip<std::complex<T>, std::complex<T>, Device>(
            d_in_real_batch + ib * nrxx,
            d_out_recip_seq + ib * npw,
            false,
            T(1.0));
    }
    ...
    // Copy results to host
    complex<T>* h_out_seq = new complex<T>[batch_count * npw];
    complex<T>* h_out_batch = new complex<T>[batch_count * npw];
    cudaMemcpy(h_out_seq, d_out_recip_seq, batch_count * npw * sizeof(complex<T>), cudaMemcpyDeviceToHost);
    cudaMemcpy(h_out_batch, d_out_recip_batch, batch_count * npw * sizeof(complex<T>), cudaMemcpyDeviceToHost);

    // Verify correctness
    T max_error = this->ComputeMaxError(h_out_seq, h_out_batch, batch_count * npw);
...
TYPED_TEST(PW_BASIS_BATCH_TEST, PartialBatch)
{
    using T = typename TestFixture::Real;
    using Device = typename TestFixture::Device;
    ...
    // Sequential (template signature: TK, TR, Device - no ik parameter)
    for (int ib = 0; ib < batch_count; ib++)
    {
        this->pwtest.template recip_to_real<std::complex<T>, std::complex<T>, Device>(
            d_in_recip_batch + ib * npw,
    ...
    // Batch with partial count (no ik_batch parameter)
    this->pwtest.template recip_to_real_batch<T, Device>(
        nullptr,
        d_in_recip_batch,
        d_out_real_batch,
