...
template <typename FPTYPE>
void xc_slater_spin(const FPTYPE rho, const FPTYPE zeta, FPTYPE& ex, FPTYPE& vxup, FPTYPE& vxdw)
{
    ...
    FPTYPE rho13 = std::pow((1.0 + zeta) * rho, third);
...
template <typename FPTYPE>
void xc_pz(const FPTYPE rs, const int iflag, FPTYPE& ec, FPTYPE& vc)
{
...
template <typename FPTYPE>
void xc_pz_polarized(const FPTYPE rs, FPTYPE& ec, FPTYPE& vc)
{
...
template <typename FPTYPE>
void xc_pz_spin(const FPTYPE rs, const FPTYPE zeta, FPTYPE& ec, FPTYPE& vcup, FPTYPE& vcdw)
{
    FPTYPE ecu = 0.0;
    FPTYPE vcu = 0.0;
    FPTYPE ecp = 0.0;
    ...
    const FPTYPE denom = std::pow(2.0, p43) - 2.0;
    const FPTYPE fz = (std::pow(1.0 + zeta, p43) + std::pow(1.0 - zeta, p43) - 2.0) / denom;
    const FPTYPE dfz = p43 * (std::pow(1.0 + zeta, third) - std::pow(1.0 - zeta, third)) / denom;
    ec = ecu + fz * (ecp - ecu);
...
template <typename FPTYPE>
void xc_pw_spin(const FPTYPE rs, const FPTYPE zeta, FPTYPE& ec, FPTYPE& vcup, FPTYPE& vcdw)
{
    const FPTYPE a = 0.0310910;
    ...
    FPTYPE olog = std::log(1.0 + 1.0 / om);
    ...
    const FPTYPE denom = std::pow(2.0, 4.0 / 3.0) - 2.0;
    const FPTYPE fz = (std::pow(1.0 + zeta, 4.0 / 3.0) + std::pow(1.0 - zeta, 4.0 / 3.0) - 2.0) / denom;
    const FPTYPE dfz = (std::pow(1.0 + zeta, 1.0 / 3.0) - std::pow(1.0 - zeta, 1.0 / 3.0))
                       * 4.0 / (3.0 * denom);
...
template <typename FPTYPE>
void xc_pbec_spin(const FPTYPE rho,
                  const FPTYPE zeta,
                  const FPTYPE grho,
                  const int iflag,
                  FPTYPE& sc,
                  ...
                  FPTYPE& v2c)
{
    ...
    const FPTYPE kf = xkf / rs;
    const FPTYPE ks = xks * std::sqrt(kf);
    const FPTYPE fz = 0.5 * (std::pow(1.0 + zeta, 2.0 / 3.0) + std::pow(1.0 - zeta, 2.0 / 3.0));
    const FPTYPE fz2 = fz * fz;
    const FPTYPE fz3 = fz2 * fz;
    const FPTYPE dfz = (std::pow(1.0 + zeta, -1.0 / 3.0) - std::pow(1.0 - zeta, -1.0 / 3.0)) / 3.0;
    const FPTYPE t = std::sqrt(grho) / (2.0 * fz * ks * rho);
    const FPTYPE expe = std::exp(-ec / (fz3 * ga));
    const FPTYPE af = be[iflag] / ga * (1.0 / (expe - 1.0));
    const FPTYPE bfup = expe * (vcup - ec) / fz3;
    const FPTYPE bfdw = expe * (vcdw - ec) / fz3;
    const FPTYPE y = af * t * t;
    const FPTYPE xy = (1.0 + y) / (1.0 + y + y * y);
    const FPTYPE qy = y * y * (2.0 + y) / std::pow(1.0 + y + y * y, 2);
    const FPTYPE s1 = 1.0 + be[iflag] / ga * t * t * xy;
    const FPTYPE h0 = fz3 * ga * std::log(s1);
...
template <typename FPTYPE>
void xc_scalar_lda_spin(const FPTYPE rho, const FPTYPE zeta, const int correlation, FPTYPE& exc, FPTYPE& vup, FPTYPE& vdw)
{
    const FPTYPE pi34 = 0.62035049089940;
    const FPTYPE rs = pi34 / std::pow(rho, 1.0 / 3.0);
    FPTYPE ex = 0.0;
