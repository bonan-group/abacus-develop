    ...
    std::complex<FPTYPE>* phase)
{
    ...
    uspp_atom_phase<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        atom_count,
        npw,
        gcar,
        tau,
    ...
    std::complex<FPTYPE>* packed)
{
    ...
    uspp_pack_becsum<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        atom_count,
        nij,
        nat,
        nh_tot,
...
{
    uspp_accumulate_rhog<FPTYPE><<<(npw + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        npw,
        nij,
    ...
    FPTYPE* becsum)
{
    ...
    uspp_becsum<FPTYPE><<<(count + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        atom_count,
        nbands,
        nh,
        nkb,
